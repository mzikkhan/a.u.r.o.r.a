...
revenue_merge <- function(input_df, csv_path) {
  ...
  # Read the revenue CSV (outside tryCatch to fail fast, or inside if we want to catch read errors only)
  # BUT tests expect error on missing column in CSV too.
  # So we'll read it, check columns, then return.
  csv_df <- tryCatch({
